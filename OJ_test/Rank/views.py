...
def Single_Rank(Current_Rank):
    ...
    Current_Summary = Summary.objects.get(Summary_Question__Question_Name=Current_Question,
                                          Summary_User__User_Name=Current_User)
...
def Rank_Attend_json(Request_Question, if_attend, UserName, UserHash):
    User_Summary = Summary.objects.filter(Summary_Question__Question_ID=Request_Question,
                                          Summary_User__User_Name=UserName)
    Return_Data = {}
    Return_Data['userName'] = UserName
    Return_Data['hash'] = UserHash
    if len(User_Summary) != 0:
        ...
        New_Rank = Summary.objects.filter(Summary_Question__Question_ID=Request_Question, Summary_Attend=True).order_by(
            'Summary_Runtime')[:100:]
        print(New_Rank)
        for i in range(len(New_Rank)):
            Rank(Rank_Question=New_Rank[i].Summary_Question, Rank_User=New_Rank[i].Summary_User,
                 Rank_Order=i + 1).save()
