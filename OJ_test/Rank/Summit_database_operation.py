...
def DataBase_Operation(Summited_Question_ID, Summited_User_Name, Summited_Output, Summited_Runtime):
    ...
    Summit(Summit_User=Current_User[0], Summit_Question=Current_Question[0], Summit_Output=Summited_Output,
           Summit_Runtime=Summited_Runtime).save()

    ''' ---updating Summary in DB--- '''
    Current_Summary = Summary.objects.filter(Summary_User__User_Name=Summited_User_Name,
                                             Summary_Question__Question_ID=Summited_Question_ID)
    if len(Current_Summary) == 0:  # if is the first time Summit this Question
        Summary(Summary_User=Current_User[0], Summary_Question=Current_Question[0],
                Summary_AC_Count=1 if Summited_Output == 'AC' else 0, Summary_Runtime=Summited_Runtime).save()
    else:
    ...
    New_Rank = Summary.objects.filter(Summary_Question__Question_ID=Summited_Question_ID, Summary_Attend=True).order_by(
        'Summary_Runtime')[:100:]
